...
function Set-JiraAttribute {
    ...
    try { $out = Invoke-WebRequest -Uri $uri  -Method PUT -Headers $Headers_jira -Body $json } # -ErrorVariable a -ErrorAction SilentlyContinue | Out-NullsenseString 
    catch { "Json = " + $json; "Error = " + $error[0] }
...
function Set-JiraAttribute2 {
    ...
    $uri = 'https://jira.' + $config.domain + '/rest/insight/1.0/object/' + $id_object + ''
    try { $out = Invoke-WebRequest -Uri $uri  -Method PUT -Headers $Headers_jira -Body $json } 
    catch { "Json = " + $json; "Error = " + $error[0] }
...
function Add-JiraObject {
    ...
    try { Invoke-WebRequest -Uri $uri  -Method POST -Headers $Headers_jira -Body $json } 
    catch { "Json = " + $json; "Error = " + $error[0] }
}
...
function Set-Log {
    ...
    $date = get-date
    $output = [string]$date + " " + $out
    out-file -FilePath $config.log_path -InputObject $output -Encoding UTF8 -Append 
}